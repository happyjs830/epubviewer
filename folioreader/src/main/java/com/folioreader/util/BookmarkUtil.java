...
public class BookmarkUtil {
    ...
    public static String createBookmarkRangy(Context context, String content, String bookId, String pageId, int pageNo, String oldRangy) {
        try {
...
//            String color = jObject.getString("color");
            ...
            String rangyBookmarkElement = getRangyString(rangy, oldRangy);
            Log.e(TAG, "rangyBookmarkElement => " + rangyBookmarkElement);

            BookmarkImpl bookmarkImpl = new BookmarkImpl();
            bookmarkImpl.setContent(textContent);
            bookmarkImpl.setPageNumber(pageNo);
            bookmarkImpl.setBookId(bookId);
            bookmarkImpl.setPageId(pageId);
            bookmarkImpl.setRangy(rangyBookmarkElement);
            bookmarkImpl.setDate(Calendar.getInstance().getTime());
            bookmarkImpl.setTimeStamp(System.currentTimeMillis() / 1000);

            // save highlight to database
            long id = BookmarkTable.insertBookmark(bookmarkImpl);
            if (id != -1) {
                bookmarkImpl.setId((int) id);
                sendBookmarkBroadcastEvent(context, bookmarkImpl, HighLight.HighLightAction.NEW);
            }
            return rangy;
    ...
    private static String getRangyString(String rangy, String oldRangy) {
        ...
        Log.e(TAG, "size1 : " + rangyList.size());
        if (rangyList.size() >= 1) {
    ...
    public static void sendBookmarkBroadcastEvent(Context context, BookmarkImpl bookmarkImpl, HighLight.HighLightAction action) {}
}
